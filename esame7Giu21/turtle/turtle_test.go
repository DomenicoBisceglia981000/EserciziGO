...
Scrivere un programma Go (il file deve chiamarsi 'turtle.go') che fa muovere una tartaruga nel piano cartesiano. La tartatura è comandata attraverso semplici comandi (vedi sotto) e può solo fare passi unitari (interi) nelle 4 direzioni cardinali: Nord, Est, Sud, Ovest (alto, destra, basso, sinistra).

Il programma deve essere dotato di:

- una struttura Turtle con due campi x, y (coordinate nel piano cartesiano) di tipo int e un campo dir (direzione N, E, S, O) di tipo byte, dichiarati in quest'ordine.
